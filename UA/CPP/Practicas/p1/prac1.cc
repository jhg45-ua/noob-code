...
/*
    Funcion: gestiona la solicitud del año academico
    strReq: año como string
    reqTmp: plantilla para imprimir solicitud
    convertToInt: true -> stoi
    return: devuelve el entero en caso de converToInt=true si no devuelve strReq
*/
void manageInput(const string &reqTmp, string &strReq, int &numReq, bool converToInt, bool checkSize)
{
    // Pedimos el año y se guarda como string
    cout << reqTmp;
    getline(cin, strReq);
    ...
    // converToInt=true
    // gestionamos el error de stoi con try-catch
    if (converToInt)
    {
        try
        {
